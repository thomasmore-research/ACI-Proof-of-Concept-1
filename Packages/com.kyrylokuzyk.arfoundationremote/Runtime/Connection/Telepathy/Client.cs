    ...
    public class Client : Common
    {
        ...
        void ReceiveThreadFunction(string ip, int port)
        {
            ...
            try
            {
                ...
                using (connection) {
                    if (connection.IsFaulted) {
                        throw new Exception($"Connection to {ip} failed with exception: {connection.Exception}");
                    }    
                }
            ...
            catch (SocketException exception)
            {
                ...
                Logger.LogWarning("Client Recv: failed to connect to ip=" + ip + " port=" + port + " reason=" + exception);
            ...
            catch (ThreadInterruptedException)
            {
                ...
                Logger.Log(nameof(ThreadInterruptedException));
            }
            catch (ThreadAbortException)
            {
                // expected if Disconnect() aborts it
                Logger.Log(nameof(ThreadAbortException));
            }
        ...
        public void Connect(string ip, int port)
        {
            ...
            // client.Client = null; // clear internal IPv4 socket until Connect(); this will produce NullReferenceException if async connection is used
        ...
        public bool IsSendingData => sendQueue.Count > 0;
    }
}
