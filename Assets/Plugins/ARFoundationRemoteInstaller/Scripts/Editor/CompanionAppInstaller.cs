    ...
    public class CompanionAppInstaller: IPreprocessBuildWithReport, IPostprocessBuildWithReport {
        ...
        static InstallerSettings settings {
            get {
                ...
                return instance != null ? instance.settings : new InstallerSettings();
            }
        }
        ...
        static CompanionAppInstaller() {
            ...
            // can be null in static constructor
        ...
        void IPostprocessBuildWithReport.OnPostprocessBuild(BuildReport report) {
            if (isBuildingCompanionApp(report)) {
                ...
                /*// adding this is not necessary, ios14.5 still works
                #if UNITY_IOS
                addLocalNetworkUsageDescription(report.summary.outputPath);
                void addLocalNetworkUsageDescription(string outputPath) {
                    var plist = new UnityEditor.iOS.Xcode.PlistDocument();
                    var path = outputPath + "/Info.plist";
                    plist.ReadFromString(File.ReadAllText(path));
                    plist.root.CreateArray("NSBonjourServices")
                        .AddString("_ARFoundationRemote._tcp");
                    plist.root.SetString("NSLocalNetworkUsageDescription", "Connection to Unity Editor.");
